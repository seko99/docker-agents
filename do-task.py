...
# Prompts
EXTRA_PROMPT_HEADER = "Дополнительные инструкции:"
...
REVIEW_PROMPT_TEMPLATE = (
    ...
    "Сверься с задачей в {jira_task_file}, дизайном design-1.md и планом plan-1.md. "
...
REVIEW_REPLY_PROMPT_TEMPLATE = (
    ...
    "дизайном design-1.md, планом plan-1.md и запиши свои комментарии в {review_reply_file}."
)
...
def usage() -> str:
  ...
  ./do-task.py review-fix [--dry] [--verbose] --items <items> [--prompt <text>] <jira-browse-url|jira-issue-key>
...
def build_config(
    ...
    *,
    ...
    if command == "review-fix" and not review_fix_points:
        raise TaskRunnerError("review-fix requires --items or an item list in interactive mode.")
...
def execute_command(config: Config) -> None:
    ...
    plan_prompt = append_extra_prompt(
        PLAN_PROMPT_TEMPLATE.format(jira_task_file=config.jira_task_file),
        config.extra_prompt,
    )
    implement_prompt = append_extra_prompt(
        IMPLEMENT_PROMPT_TEMPLATE,
        config.extra_prompt,
    )
    ...
    if config.command == "implement":
        ...
        implement_env["CODEX_PROMPT"] = implement_prompt
        implement_env["CODEX_EXEC_FLAGS"] = f"--model {shlex.quote(codex_model())} --dangerously-bypass-approvals-and-sandbox"
        print_info("Running Codex implementation mode")
        print_prompt("Codex", implement_prompt)
        run_command(
            docker_compose_cmd
            + [
                "-f",
    ...
    if config.command == "review":
        ...
        review_summary_file = f"review-summary-{iteration}.md"
        review_reply_summary_file = f"review-reply-summary-{iteration}.md"
        claude_prompt = append_extra_prompt(
        ...
        codex_reply_prompt = append_extra_prompt(
    ...
    if config.command == "review-fix":
        ...
        review_fix_prompt = append_extra_prompt(
        ...
        fix_env["CODEX_EXEC_FLAGS"] = f"--model {shlex.quote(codex_model())} --dangerously-bypass-approvals-and-sandbox"
        print_info(f"Running Codex review-fix mode (iteration {latest_iteration})")
...
def summarize_task(jira_ref: str) -> tuple[str, str]:
    ...
    summary_text = run_claude_summary(claude_cmd, TASK_SUMMARY_FILE, summary_prompt)
...
def parse_interactive_command(line: str, jira_ref: str) -> Config | None:
    ...
    if command_name == "review-fix":
        if len(parts) < 2:
            raise TaskRunnerError("review-fix requires item numbers, for example: /review-fix 1,2,3")
        return build_config(
            command_name,
            jira_ref,
            review_fix_points=parts[1],
            extra_prompt=" ".join(parts[2:]) or None,
        )
